...
class Depth3DPerFrame(nn.Module):
    """
    Conv3D ligero que NO colapsa el tiempo.
    Entrada:  [B,1,T,H,W]
    Salida:   [B,T,D_depth]
    """
    ...
    def forward(self, depth_btchw: torch.Tensor) -> torch.Tensor:
        # depth_btchw: [B,1,T,H,W]
        feat = self.net(depth_btchw)          # [B,C,T,H',W']
        feat = feat.mean(dim=[3, 4])          # [B,C,T]
        feat = feat.permute(0, 2, 1)         # [B,T,C]
        return self.proj(feat)               # [B,T,D_depth]


class VJEPA2DepthTransformerBinary(nn.Module):
    """
    Backbone V-JEPA 2 + Depth3DPerFrame + TransformerPerFrame (frame-wise).

    - V-JEPA: embeddings RGB por frame (via tubelets + upsampling).
    - Depth3DPerFrame: embeddings depth por frame.
    - Fusión: concat en dim de features.
    - Temporal head: logits por frame (TransformerPerFrame).
    - ANYTIME por ventana: max_t(logits frame).
    """
    def __init__(
        self,
        hf_repo: str,
        ...
        super().__init__()
        print(f"[MODEL] Cargando backbone HF: {hf_repo}")
        ...
        # opcional: cargar encoder ckpt (no se usa en eval, pesos ya vienen del checkpoint)
        if encoder_ckpt:
        ...
        # por defecto congelado; en eval no tocamos nada
        ...
        # Temporal head = TransformerPerFrame (igual que en train)
    ...
    def _rgb_embeddings_per_frame(self, pixel_values_videos: torch.Tensor) -> torch.Tensor:
        """
        Convierte tokens de V-JEPA a [B,T,D_rgb].
        """
        out = self.vjepa2(pixel_values_videos=pixel_values_videos, output_hidden_states=False)
        tokens = out.last_hidden_state  # [B,L,D]
        B, L, D = tokens.shape

        expected_L = self.steps * (self.grid ** 2)
        if L != expected_L:
            # asumimos CLS en primera posición
            tokens = tokens[:, 1:, :]
            L = tokens.shape[1]
            if L != expected_L:
                raise RuntimeError(
                    f"[ERR] Longitud de tokens inesperada: {L} vs {expected_L}"
                )

        # [B,steps,grid^2,D] -> pool espacial -> [B,steps,D]
        seq_steps = tokens.view(B, self.steps, self.grid ** 2, D).mean(dim=2)

        # Upsample temporal a frames_per_clip
        seq_rgb = F.interpolate(
            seq_steps.transpose(1, 2),  # [B,D,steps]
    ...
    def forward(
        ...
        depth_videos: torch.Tensor,
        return_frame_logits: bool = False,
    ) -> torch.Tensor:
        # RGB
        z_rgb = self._rgb_embeddings_per_frame(pixel_values_videos)  # [B,T,D_rgb]

        # Depth
        z_depth = self.depth_net(depth_videos)  # [B,T,D_depth]

        # Alinear T por seguridad
        if z_depth.shape[1] != z_rgb.shape[1]:
            T_target = z_rgb.shape[1]
            z_depth = F.interpolate(
                z_depth.transpose(1, 2),
                size=T_target,
                mode="linear",
                align_corners=False,
            ).transpose(1, 2)

        z_fused = torch.cat([z_rgb, z_depth], dim=-1)  # [B,T,D_fused]
        B, T, _ = z_fused.shape
        mask = torch.ones(B, T, dtype=torch.bool, device=z_fused.device)

        logits_bt = self.temporal_head(z_fused, mask=mask)  # [B,T,1]
        logits_bt = logits_bt.squeeze(-1)                   # [B,T]
...
def main():
    ...
    model = VJEPA2DepthTransformerBinary(
        ...
        depth_dim=depth_dim,
        unfreeze_blocks=0,
        encoder_ckpt=None,  # Pesos ya están en el checkpoint
    )
